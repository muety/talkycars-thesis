...
The purpose of this chapter is to pick up the previously presented concepts and techniques and explain how they were implemented in software. This includes a discussion about various technological choices, the use of appropriate software design patterns and essential performance-related considerations. \autoref{fig:components_full} picks the component diagram from \autoref{subsec:concept_design:components_overview} up again, but includes implementation-specific technologies in addition. 
...
\autoref{fig:client_classes} depicts core sub-modules of the simulation client. In order to achieve a high degree of \textbf{modularity} and keep \textbf{clean code boundaries} between simulation-related code and the actual CP system, different abstractions are made.
...
The second type of parameters are used to specify details about the current scene to simulate.
...
\begin{table}[H]
	...
	\begin{tabular}{|p{7.6cm}|p{7.6cm}|}
		\hline
		\textbf{Parameter} & \textbf{Description} \\ \hline
		\texttt{MAP} & Carla map or environment to run (see \autoref{fig:carla_collage}) \\ \hline
...
This chapter discussed various details about the concrete implementations of concepts and components presented in \autoref{ch:concept_design}. The resulting software system is subsequently used for the evaluation in \autoref{ch:evaluation} and constitutes a proof-of-concept for a novel, holistic cooperative perception system. In order to use it in real-world scenarios, one would have to add thorough quality assurance and testing as well as several performance optimizations. The system is built following a modular approach to make it as easy as possible to re-use or replace certain parts, while keeping others. Accordingly, the simulation client might be switched out by a component interfacing with a real AD pipeline without too much effort. As this implementation is published as an open-source project\footnote{\url{https://github.com/n1try/talkycars-thesis}}, it can be used openly as a based for further research. 