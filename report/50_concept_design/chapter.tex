...
These needs lead to the introduction of \textbf{Probabilistic Entity Relationship Models} (PER models). Such are already used by \cite{Petrich2018} for traffic scene representation and can be seen as an extension to regular Entity Relationship Models (ER models). Correspondingly, they incorporate the notion of entities, which have attributes and inherently define entity-entity relations. Entity-attribute combinations and entity-entity relations can be represented as $\langle subject, predicate, object \rangle$ triples:

$$r \in \{ \langle e_1, r, e_2 \rangle | e_{\{1,2\}} \in E, r \in R \} \cup \{ \langle e, a, \mathcal{L} \rangle | e \in E, a \in A \}$$

$E$ is the set of all entities, $A$ is the set of all attributes, $R$ is the set of all entity-entity relations and $\mathcal{L}$ is a literal, i.e. a number, boolean value or string.

With PER models, every triple is extended with an additional confidence parameter and therefore becomes a \textbf{quadruple} of $\langle subject, predicate, object, \textit{conf} \rangle$: 
...
For example, an excerpt of a PER model instance, that was observed by some ego vehicle, might potentially look like this:
...
In the example, $\textit{obstacle\_25}$ and $\textit{traffic\_light\_190}$ are entities, which were perceived, identified and tracked by an intelligent vehicle's sensory. $\textit{isOfType}$ and $\textit{hasVelocity}$ are attribute relations with literal attributes (string and three-dimensional numeric vector) and $\textit{isStoppedAt}$ describes an entity-entity relation.

Using quadruples, a PER model is representable as a graph and thus can quite easily be searched, transformed and extended by inserting new relations. This allows for a variety of potential future work, e.g. concerning with efficient graph representation as tensors \cite{Petrich2018}. The graphical representation also allows for different types of inference. New facts about the local world could be inferred when used in combination with first-order logic. Also, situations might easily be compared for case-based reasoning. \cite{Nickel2016} presents a multitude of further techniques to perform relational machine-learning on knowledge graphs, which might prove advantageous in AD contexts as well. 
